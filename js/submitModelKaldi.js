...
//		var workerNumber = $("input[name='worker_number']").val();
		var runfiles = $("input[name='runfiles']").val();
	//	var entrypoint = $("input[name='entrypoint']").val();
		...
		/*else if(isNaN(workerNumber)){
			
			alert("工作结点数目输入错误");
		}*/
		else if(runfiles=="请输入目录路径"){
			alert("训练样本路径输入错误");
		}
...
//			alert("too many gpu ask".indexOf(flag));
			if("gpu exceed all gpu".indexOf(flag)+1>0){
				...
				alert("添加成功");
				window.location.href="taskManagerKaldi.jsp";
			}
//			 alert("添加成功");
			//setTimeout("layer.alert('添加成功', 8)",1000);
			//Pause(this,20000);//调用暂停函数
...
function checkWorkerNumber(obj){
//	var maxgpu=$("#hiddengpu").val();
	...
	if(isNaN(obj.value)){
		$('#note3').text(obj.defaultValue);
//		alert(obj.value);
	}/*else{
		var worker = obj.value;
		$('#note3').text("");
		if(worker<0){
			$('#note3').text('请求的工作结点数目错误');
		}
	}*/
}
...
// if (!reg.test(name)) {
		$.ajax({
			type : "post",
			dataType : "json",
			url : "/CMRI/task/getUserTaskInfoKaldi",//"/NeoMalbox/task/checkName",
			data : {
				username : username,//
				taskName : name
			},
