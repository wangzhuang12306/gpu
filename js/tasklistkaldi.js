	...
	$.ajax({
		...
		success : function(msg) {
			...
			$.each(msg, function(idx, item) {
				...
				row.find(".worker").text(item.workerNumber);
				row.find(".runsh").text(item.runfiles);
	...
	row.find("#startTask").click(function() {
		...
		if (isSure) {
				var id = $(this).parents("tr").find(".prop_id").text();
					...
					var worker = $(this).parents("tr").find(".worker").text();
					var runsh = $(this).parents("tr").find(".runsh").text();
					var datadir = $(this).parents("tr").find(".datadir").text();
					$.ajax({
						type : "post",
						dataType : "text",
						url : "/CMRI/task/startkaldi",
						data : {
							id : id,
							taskName : taskName,
							gpuNumber : gpuNumber,
...
//					console.log($(this).parents("tr").find("#pluginurl").attr("id"));
					var url2 = "/CMRI/download/res?username="+user+"&taskname="+taskName+"&kind=kaldi";//////
...
//							 alert("flag: "+flag);
	...
	row.find("#testkaldi").click(function() {
		var isSure = confirm("确认？");
		if (isSure) {
					var id = $(this).parents("tr").find(".prop_id").text();
					var taskName = $(this).parents("tr").find(".prop_taskname").text();
					var user = $(this).parents("tr").find(".prop_username").text();
